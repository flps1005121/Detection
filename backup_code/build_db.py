...
class ImageFeatureExtractor:
    ...
    def create_features_database(self, train_dir, db_file='train_features.db', force_refresh=True):
        """為訓練集中所有圖像創建原型特徵資料庫，只儲存類別原型，不儲存個別圖像特徵"""
        ...
        print(f"正在為 {len(dataset)} 張訓練圖像提取特徵以構建原型...")
        features = []
        valid_file_paths = []
        labels = []
        ...
        # 計算每個類別的原型特徵向量（平均特徵向量）
        print("計算類別原型...")
        class_prototypes = {}
        for class_name in set(labels):
            # 收集該類別的所有特徵
            class_features = [features[i] for i in range(len(features)) if labels[i] == class_name]
            # 計算平均特徵作為原型
            prototype = np.mean(class_features, axis=0)
            class_prototypes[class_name] = prototype
            print(f"  - 類別 '{class_name}' 的原型已計算，使用了 {len(class_features)} 個樣本")
        ...
        try:
            ...
            # 創建原型特徵表
            cursor.execute('''CREATE TABLE IF NOT EXISTS prototypes
            ...
            for class_name, prototype in class_prototypes.items():
                # 計算該類別的樣本數量
                sample_count = labels.count(class_name)
                cursor.execute("INSERT INTO prototypes (label, feature, sample_count) VALUES (?, ?, ?)",
                              (class_name, prototype.tobytes(), sample_count))
                              
            # 提交更改並關閉連接
            conn.commit()
            conn.close()
            
            print(f"原型資料庫已儲存到 {os.path.abspath(db_file)}")
            print(f"資料庫包含 {len(dataset.classes)} 個類別: {dataset.classes}")
        except Exception as e:
            print(f"儲存原型資料庫時發生錯誤: {e}")
