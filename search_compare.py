...
def predict_and_display(test_dir, db_file='train_features.db', log_file=None):
    """預測並顯示結果，可選擇性地記錄到檔案中"""
    # 記錄開始時間
    ...
    # 使用ImageFolder處理測試圖像
    log_to_file(log_file, f"開始判斷 {len(test_dataset)} 張測試圖像...")
    results = []
    ...
    for idx, (img, class_idx) in enumerate(tqdm(test_loader)):
        ...
        # 使用原始圖像路徑進行處理，傳入標籤參數
        process_test_image(img_path, img_name, train_features, train_file_paths, train_labels, extractor, results, log_file, class_name)
...
if __name__ == "__main__":
    ...
    # 執行相似圖像預測
    print(f"\n執行相似圖像預測...\n結果將保存到: {log_file}")
    predict_and_display(test_dir, db_file, log_file)