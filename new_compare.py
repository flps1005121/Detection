...
def predict_and_display(test_dir, top_k=5, db_file='train_features.db', log_file=None, selected_label=None):
    """預測並顯示結果，可選擇性地記錄到檔案中，並可根據標籤過濾比對結果"""
    # 記錄開始時間
    ...
    try:
        ...
        if selected_label:
            log_to_file(log_file, f"成功從 {db_file} 載入 {len(train_file_paths)} 個特徵向量 (標籤: '{selected_label}')")
        else:
            log_to_file(log_file, f"成功從 {db_file} 載入 {len(train_file_paths)} 個特徵向量 (所有標籤)")
    except Exception as e:
    ...
    if len(train_features) == 0:
        if selected_label:
            log_to_file(log_file, f"無法繼續：找不到標籤為 '{selected_label}' 的特徵資料")
        else:
...
if __name__ == "__main__":
    ...
    # 傳入標籤參數
    predict_and_display(test_dir, top_k, db_file, log_file, selected_label)